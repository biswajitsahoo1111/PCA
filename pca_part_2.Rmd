...
```
Our data matrix contains 12 data points. So to find variance of principal components we have to divide the square of the diagonal matrix by 11. To know the theory behind it, refer [Part-I](https://biswajitsahoo1111.wordpress.com/2018/12/28/principal-component-analysis---part-i/)

### Performing covariance PCA using Eigen-decomoposition(Not recommended)
This procedure is not recommended because forming a covariance matrix is computaionally not efficient for large matrices if data matrix contins smaller entries. So doing eigen analysis on covariance matrix may give erroneous results. However, for our example we can use it to obtain results.
```{r}
